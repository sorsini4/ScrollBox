...
1) Figure out how to display a message more than once.
   ...
   Note that the wordscroll.c program uses a loop; this display_string()
...
2) Add code to handle keystrokes and mouse clicks.  Note that you
...
a) Add smooth scrolling.
   ...
   Right now, the scrollbox jumps one letter at a time, because it copies
   in full bytes.  A more attractive way for it to work would be to step
   the display one pixel at a time, instead of in blocks.
